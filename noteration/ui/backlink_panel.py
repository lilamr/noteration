...
Displays:
  - List of notes linking TO the active note (backlinks)
  - List of notes linked FROM the active note (forward links)
...
class BacklinkPanel(QWidget):
    ...
    Signals
    -------
    note_requested(stem: str)  — user clicked a note in the list
    ...
    def _make_stats_widget(self) -> QWidget:
        ...
        btn_rebuild = QPushButton("↺  Rebuild Graph")
    ...
    def set_current_note(self, note_stem: str) -> None:
        """Called when the user switches to a different note."""
    ...
    def refresh_all(self) -> None:
        """Refresh the entire panel (after graph rebuild)."""
        self._refresh_note_lists()
        self._refresh_stats()
        self._refresh_orphans()

        # Update badge if there is an active note
    ...
    def _refresh_note_lists(self) -> None:
        ...
        for nid in backs:
            ...
            item.setToolTip(f"[[{nid}]] links here")
