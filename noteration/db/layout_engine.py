...
Algorithm:
  - Repulsion: nodes repel each other (Coulomb inverse-square)
  - Attraction: connected nodes pull together (Hooke's spring)
  - Centering: pull towards the viewport center
"""
...
class LayoutEngine:
    ...
    def __init__(
        ...
        self.repulsion_cutoff: float = 250.0  # Cutoff distance for O(N^2) repulsion optimization
    ...
    def iterate(self, iterations: int = 1) -> Iterator[dict[str, Point]]:
        """Run one or more layout iterations and yield node positions."""
        ...
        for _ in range(iterations):
            ...
            for i, node_a in enumerate(nodes):
                ...
                # Optimized repulsion calculation with cutoff
            ...
            # Update node positions and apply cooling/damping
            for node in nodes:
                fx, fy = forces[node]
                vx, vy = self._velocities[node]

                # Apply temperature-scaled forces
    ...
    def get_positions(self) -> dict[str, Point]:
        """Return the current mapping of nodes to (x, y) coordinates."""
    ...
    def center_graph(self) -> None:
        """Re-center all nodes within the current viewport."""
    ...
    def initialize_from_adj(self, adj: dict[str, set[str]]) -> None:
        """Initialize node positions in a circle from an adjacency dictionary."""
        self._adj = adj
