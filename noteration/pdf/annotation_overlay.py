...
Transparent QWidget placed on top of a PDF page to display:
  ...
  - Comments (sticky note icon)
  - Bookmarks (bookmark icon)

Interaction:
  - Right-click on overlay → context menu (delete, edit, jump to note)
  - Drag to create a new highlight (when highlight mode is active)
  - Click to add a comment (when comment mode is active)
"""
...
class AnnotationOverlay(QWidget):
    ...
    def _paint_annotation(self, painter: QPainter, ann: Annotation) -> None:
        if ann.type in ("highlight", "image"):
            ...
            # Prioritize quads for highlight (text)
            ...
            # Small tag label if a note exists
        ...
        if self._dragging:
            ...
            if self._mode == "highlight":
                if self._drag_start_idx != -1 and self._drag_end_idx != -1:
                    ...
                    # FALLBACK: If no word detected, use box selection
                    drag_rect = QRectF(self._drag_start_pos, self._drag_end_pos).normalized()
    ...
    def _show_context_menu(self, global_pos, ann: Annotation | None) -> None:
        ...
        if ann:
            ...
            menu.addSeparator()
            if ann.linked_notes:
                jump_act = menu.addAction("Jump to Linked Note")
            else:
                jump_act = None
        else:
            edit_act = None
...
class CommentDialog(QDialog):
    """Simple dialog for inputting / editing annotation note text."""

    def __init__(self, parent=None, initial_text: str = "", read_only: bool = False) -> None:
        super().__init__(parent)
        self.setWindowTitle("Annotation Note")
        self.resize(360, 180)
